...
KERNELVERSION_X86 := 	4.14.15+linaro
...
ifeq ($(CONFIG_MULTI_INTERFACE),y)
	cp -f mlan_usb.$(MODEXT) $(BINDIR)/mlan_usb$(DBG).$(MODEXT)
else
	cp -f mlan.$(MODEXT) $(BINDIR)/mlan$(DBG).$(MODEXT)
endif
	mv usb8xxx.$(MODEXT) $(BINDIR)/usb8997$(DBG).$(MODEXT)
	cp -rpf script/usb_x86/* $(BINDIR)/
	cp -rpf script/usb/* $(BINDIR)/
...
install: default
	...
	cp -f usb8xxx.$(MODEXT) $(INSTALLDIR)/usb8997$(DBG).$(MODEXT)
	echo $(INSTALLDIR)
	echo "usb8997 Driver Installed"
