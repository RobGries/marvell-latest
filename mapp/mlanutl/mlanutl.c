...
#endif
static int process_heart_beat(int argc, char *argv[]);
 ...
 */
static int
process_heart_beat(int argc, char *argv[])
{
	int ret = 0, data[3];
	t_u8 *buffer = NULL;
	struct eth_priv_cmd *cmd = NULL;
	...
	if (argc != 3 && argc != 6) {
		printf("Error: invalid no of arguments\n");
		printf("mlanutl mlanX heartbeat [<hTime> <dTime> <delayTime>]\n");
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
	...
	if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
		perror("mlanutl");
		fprintf(stderr, "mlanutl: heartbeat fail\n");
		ret = MLAN_STATUS_FAILURE;
		goto done;
	}
