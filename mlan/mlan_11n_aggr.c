		       ...
		       int headroom, int ptrindex)
{
	...
	pusb_tx_aggr = wlan_get_usb_tx_aggr_params(pmadapter, priv->port);
	if (pusb_tx_aggr) {
		tx_param.next_pkt_len = ((pmbuf_src) ?
					 pmbuf_src->data_len +
					 sizeof(TxPD) : 0);
		ret = wlan_usb_host_to_card_aggr(pmadapter, pmbuf_aggr,
						 &tx_param, pusb_tx_aggr);
	} else {
