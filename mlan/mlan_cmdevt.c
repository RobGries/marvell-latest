		   ...
		   IN t_void *pioctl_buf, IN t_void *pdata_buf)
{
	...
	pcmd_node->cmdbuf = wlan_alloc_mlan_buffer(pmadapter,
...
wlan_free_cmd_buffer(IN mlan_adapter *pmadapter)
{
	...
	for (i = 0; i < MRVDRV_NUM_OF_CMD_BUFFER; i++) {
		...
		if (pcmd_array[i].respbuf) {
			pmadapter->callbacks.moal_recv_complete(pmadapter->
								pmoal_handle,
								pcmd_array[i].
								respbuf,
								pmadapter->
		...
		for (interfaces = 0; interfaces < num_intf; interfaces++) {
			...
			intf_priv =
				wlan_get_priv_by_id(pmadapter, bss_num,
						    bss_type);
			if (intf_priv) {
				if (pmulti_chan_grp_info->hid_num.usb_epnum ==
				    MLAN_USB_EP_DATA ||
				    pmulti_chan_grp_info->hid_num.usb_epnum ==
 ...
 */
mlan_status
wlan_cmd_packet_aggr_ctrl(IN pmlan_private pmpriv,
			  IN HostCmd_DS_COMMAND *cmd,
			  IN t_u16 cmd_action, IN t_void *pdata_buf)
{
	...
	cmd->size =
		wlan_cpu_to_le16(sizeof(HostCmd_DS_PACKET_AGGR_CTRL) +
				 S_DS_GEN);
 ...
 */
mlan_status
wlan_ret_packet_aggr_ctrl(IN pmlan_private pmpriv,
			  IN HostCmd_DS_COMMAND *resp,
			  IN mlan_ioctl_req *pioctl_buf)
{
	mlan_ds_misc_cfg *misc = MNULL;
	...
	if (aggr_ctrl->aggr_enable & MBIT(0)) {
		if (!pmadapter->usb_tx_aggr[0].aggr_ctrl.enable) {
			pmadapter->usb_tx_aggr[0].aggr_ctrl.enable = MTRUE;
			change = MTRUE;
		}

	} else {
		if (pmadapter->usb_tx_aggr[0].aggr_ctrl.enable) {
			pmadapter->usb_tx_aggr[0].aggr_ctrl.enable = MFALSE;
			change = MTRUE;
		}
	}
	pmadapter->usb_tx_aggr[0].aggr_ctrl.aggr_mode =
		MLAN_USB_AGGR_MODE_LEN_V2;
	pmadapter->usb_tx_aggr[0].aggr_ctrl.aggr_align =
	...
	if (change) {
		...
		for (i = 0; i < pmadapter->priv_num; ++i) {
			if (pmadapter->priv[i]) {
				pusb_tx_aggr =
					wlan_get_usb_tx_aggr_params(pmadapter,
								    pmadapter->
								    priv[i]->
								    port);
				if (pusb_tx_aggr &&
