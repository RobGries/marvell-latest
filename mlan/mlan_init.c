...
wlan_init_priv(pmlan_private priv)
{
	...
	if (pusb_tx_aggr &&
	    pusb_tx_aggr->aggr_ctrl.aggr_mode == MLAN_USB_AGGR_MODE_LEN_V2) {
...
wlan_init_adapter(pmlan_adapter pmadapter)
{
	...
	for (i = 0; i < MAX_USB_TX_PORT_NUM; i++) {
		...
		pmadapter->usb_tx_aggr[i].port = pmadapter->usb_tx_ports[i];
...
wlan_init_timer(IN pmlan_adapter pmadapter)
{
	...
	if (pcb->
	    moal_init_timer(pmadapter->pmoal_handle,
			    &pmadapter->hb_device_timer, wlan_device_heart_beat,
			    pmadapter)
	    != MLAN_STATUS_SUCCESS) {
		ret = MLAN_STATUS_FAILURE;
		goto error;
