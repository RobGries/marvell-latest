			 ...
			 IN pmlan_ioctl_req pioctl_req)
{
	...
	if (pioctl_req->action == MLAN_ACT_GET) {
		...
		debug_info->num_card_to_host_failure =
...
wlan_host_heart_beat(void *function_context)
{
	...
	if ((pmadapter->is_hb_ignored == MFALSE)
		...
		if ((pmadapter->host_to_dev_traffic <= 1)
			...
			wlan_prepare_cmd(wlan_get_priv
			  ...
			  IN pmlan_ioctl_req pioctl_req)
{
	...
	ret = wlan_prepare_cmd(pmpriv,
			       ...
			       cmd_action,
			       0,
			       (t_void *)pioctl_req, &misc->param.aggr_params);
...
push_n(pmlan_adapter pmadapter, mef_stack *s, t_u8 len, t_u8 *val)
{
	if ((s->sp + len) <= MAX_NUM_STACK_BYTES) {
		memcpy(pmadapter, s->byte + s->sp, val, len);
		s->sp += len;
		return MLAN_STATUS_SUCCESS;
	} else {
...
mef_push(pmlan_adapter pmadapter, mef_stack *s, mef_op *op)
{
	...
	case OPERAND_BYTE_SEQ:
		...
		if (MLAN_STATUS_SUCCESS ==
		    ...
		    MLAN_STATUS_SUCCESS == push_n(pmadapter, s, 1,
						  &op->operand_type))
			ret = MLAN_STATUS_SUCCESS;
		else
			ret = MLAN_STATUS_FAILURE;
...
push_filter_dnum_eq(pmlan_adapter pmadapter, mef_stack *s, mef_filter_t *filter)
{
	...
	dnum = filter->pattern;
	memcpy(pmadapter, op.val, &dnum, sizeof(dnum));
...
push_filter_byte_eq(pmlan_adapter pmadapter, mef_stack *s, mef_filter_t *filter)
{
	...
	memcpy(pmadapter, &op.val[1], filter->byte_seq, filter->num_byte_seq);
	ret = mef_push(pmadapter, s, &op);
	if (ret != MLAN_STATUS_SUCCESS)
		goto done;
...
push_filter_bit_eq(pmlan_adapter pmadapter, mef_stack *s, mef_filter_t *filter)
{
	...
	memcpy(pmadapter, &op.val[1], filter->mask_seq, filter->num_mask_seq);
	ret = mef_push(pmadapter, s, &op);
	if (ret != MLAN_STATUS_SUCCESS)
		goto done;
 ...
 */
mlan_status
wlan_generate_mef_filter_stack(pmlan_adapter pmadapter, mef_stack *s,
			       mef_entry_t *entry)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
	mef_op op;
...
*/
mlan_status
wlan_set_mef_entry(IN mlan_private *pmpriv,
		   IN pmlan_adapter pmadapter, IN mef_cfg *pmef)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
 ...
 */
mlan_status
wlan_process_mef_cfg_cmd(IN mlan_private *pmpriv, IN pmlan_adapter pmadapter)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
	pmlan_callbacks pcb;
	mef_cfg mef;
	...
	if (pmef->enable_autoarp_entry) {
		memcpy(pmadapter, pentry, &pmef->entry[5], sizeof(mef_entry_t));
		pentry += pmef->enable_autoarp_entry;
	}
    /** Copy wowlan entry*/
	if (pmef->num_wowlan_entry) {
		memcpy(pmadapter, pentry, &pmef->entry[6], sizeof(mef_entry_t));
		pentry += pmef->num_wowlan_entry;
	}
    /** Copy IPv6 NS message offload entry */
	if (pmef->num_ipv6_ns_offload)
		memcpy(pmadapter, pentry, &pmef->entry[7], sizeof(mef_entry_t));
 ...
 */
mlan_status
wlan_misc_ioctl_mef_flt_cfg(IN pmlan_adapter pmadapter,
			    IN pmlan_ioctl_req pioctl_req)
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
	mlan_ds_misc_cfg *misc_cfg = MNULL;
	...
	case MLAN_ACT_GET:
		if (mef_cfg->mef_act_type == MEF_ACT_WOWLAN)
			memcpy(pmadapter, &mef_cfg->mef_entry,
