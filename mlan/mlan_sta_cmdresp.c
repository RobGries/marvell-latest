	...
	if (pioctl_buf) {
		while (tlv_buf_len > 0) {
			...
			case MRVL_USB_AGGR_PARAM_TLV_ID:
				usb_aggr_param_tlv =
					(MrvlIETypes_USBAggrParam_t *)
					ptlv_buffer;
				misc = (mlan_ds_misc_cfg *)pioctl_buf->pbuf;
				usb_aggr_ctrl =
					(mlan_ds_misc_usb_aggr_ctrl *)&(misc->
									param.
									usb_aggr_params);
				...
				if (pioctl_buf->action == MLAN_ACT_SET) {
					...
					for (i = 0; i < MAX_USB_TX_PORT_NUM;
					     i++) {
						pmadapter->usb_tx_aggr[i].
							aggr_ctrl.enable =
							usb_aggr_ctrl->
							tx_aggr_ctrl.enable;
						if (pmadapter->usb_tx_aggr[i].
						    aggr_ctrl.aggr_mode !=
				...
				if (pioctl_buf->action == MLAN_ACT_SET) {
					...
					pmadapter->usb_rx_deaggr.aggr_ctrl.
						...
						enable;
					if (pmadapter->usb_rx_deaggr.aggr_ctrl.
					    aggr_mode !=
					    usb_aggr_ctrl->rx_deaggr_ctrl.
					    aggr_mode)
						pmadapter->usb_rx_deaggr.
							aggr_ctrl.aggr_mode =
							usb_aggr_ctrl->
							rx_deaggr_ctrl.
							aggr_mode;
