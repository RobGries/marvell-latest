...
wlan_ops_uap_process_rx_packet(IN t_void *adapter, IN pmlan_buffer pmbuf)
{
	...
	if (ret || (rx_pkt_type == PKT_TYPE_BAR)) {
		ret = pmadapter->callbacks.moal_recv_complete(pmadapter->
							      pmoal_handle,
							      pmbuf,
							      pmadapter->
							      rx_data_ep, ret);
		if (ret)
			PRINTM(MERROR,
			       "uAP Rx Error: moal_recv_complete returned error\n");
	}
done:
	LEAVE();
	return ret;
				...
				if (newbuf) {
					...
					newbuf->data_offset =
						(sizeof(UapTxPD) +
						 priv->intf_hr_len +
						 DMA_ALIGNMENT);
					util_scalar_increment(pmadapter->
							      pmoal_handle,
							      &pmadapter->
							      pending_bridge_pkts,
							      pmadapter->
					...
					newbuf->data_len =
						pmbuf->data_len -
						prx_pd->rx_pkt_offset;
					wlan_wmm_add_buf_txqueue(pmadapter,
								 newbuf);
					if (util_scalar_read
