				  ...
				  pmlan_private *priv, int *tid)
{
	...
	for (j = pmadapter->priv_num - 1; j >= 0; --j) {
		...
		do {
			...
			if (!wlan_is_port_ready
			    (pmadapter, priv_tmp->port_index)) {
				PRINTM(MINFO,
				       "get_highest_prio_ptr(): "
				       "usb port is busy,Ignore pkts from BSS%d\n",
				       priv_tmp->bss_index);
				/* Ignore data pkts from a BSS if usb port is busy */
				goto next_intf;
			}
	...
	if (pmbuf) {
		...
		if (pusb_tx_aggr) {
			tx_param.next_pkt_len = ((pmbuf_next) ?
						 pmbuf_next->data_len +
						 sizeof(TxPD) : 0);
			ret = wlan_usb_host_to_card_aggr(pmadapter, pmbuf,
							 &tx_param,
							 pusb_tx_aggr);
		} else {
			pmadapter->usb_port_status[priv->port_index] = MTRUE;

			ret = pmadapter->callbacks.
				moal_write_data_async(pmadapter->pmoal_handle,
						      pmbuf, priv->port);
		}
		switch (ret) {
		...
		case MLAN_STATUS_FAILURE:
			pmadapter->usb_port_status[priv->port_index] = MFALSE;
		...
		case MLAN_STATUS_PENDING:
			if (!pusb_tx_aggr)
				pmadapter->usb_port_status[priv->port_index] =
					MFALSE;
			break;
