...
woal_priv_set_get_aggrctrl(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	...
	memset((char *)data, 0, sizeof(data));
	if (strlen(respbuf) ==
	    (strlen(CMD_MARVELL) + strlen(PRIV_CMD_AGGRCTRL))) {
		/* GET operation */
		user_data_len = 0;
	} else {
	...
	memcpy(respbuf, (t_u8 *)&pcfg_misc->param.aggr_params,
	       sizeof(mlan_ds_misc_aggr_ctrl));
			      ...
			      t_u32 respbuflen)
{
	...
	memset((char *)data, 0, sizeof(data));
	if (strlen(respbuf) ==
	    (strlen(CMD_MARVELL) + strlen(PRIV_CMD_USBAGGRCTRL))) {
		/* GET operation */
		user_data_len = 0;
	} else {
		/* SET operation */
 ...
 */
static int
woal_priv_heart_beat(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	int data[3];
	int ret = 0;
	mlan_ioctl_req *req = NULL;
	...
	header_len = strlen(CMD_MARVELL) + strlen(PRIV_CMD_HEART_BEAT);
	...
	if (strncmp(buf, CMD_MARVELL, strlen(CMD_MARVELL)) == 0) {
		...
		if (strnicmp
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_AGGRCTRL,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_USB_SUSPEND,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_USB_RESUME,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_HEART_BEAT,
			    strlen(PRIV_CMD_HEART_BEAT)) == 0) {
			/* Enable/disable Heart beat feature */
			len = woal_priv_heart_beat(priv, buf,
						   priv_cmd.total_len);
			goto handled;
#if defined(STA_SUPPORT)
		} else if (strnicmp
