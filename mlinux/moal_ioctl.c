...
woal_set_auto_arp_ext(moal_handle *handle, t_u8 enable)
{
	...
	for (i = 0; i < handle->priv_num && (priv = handle->priv[i]); i++) {
		if (priv && priv->ip_addr_type != IPADDR_TYPE_NONE) {
			memcpy(ip_addr[ip_addr_num], priv->ip_addr, IPADDR_LEN);
			ip_addr_num++;
		}
	}
	if (ip_addr_num == 0) {
	...
	memcpy(filter->byte_seq, "\x08\x06", 2);
	...
	for (i = 0; i < ip_addr_num; i++) {
		...
		memcpy(filter->byte_seq, &ip_addr[i], sizeof(t_u32));
		if (i > 1)
 ...
 */
mlan_status
woal_set_get_wowlan_config(moal_private *priv, t_u16 action, t_u8 wait_option,
			   mlan_ds_misc_mef_flt_cfg *mefcfg)
{
	...
	req->req_id = MLAN_IOCTL_MISC_CFG;
	req->action = action;
	memcpy(&misccfg->param.mef_flt_cfg, mefcfg,
	       sizeof(mlan_ds_misc_mef_flt_cfg));
	/* Send IOCTL request to MLAN */
	ret = woal_request_ioctl(priv, req, wait_option);
	if (ret == MLAN_STATUS_SUCCESS) {
		if (mefcfg && action == MLAN_ACT_GET) {
			memcpy(mefcfg, &misccfg->param.mef_flt_cfg,
			       sizeof(mlan_ds_misc_mef_flt_cfg));
		}
	}
	if (ret != MLAN_STATUS_SUCCESS && ret != MLAN_STATUS_PENDING)
