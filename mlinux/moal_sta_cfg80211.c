		       ...
		       mlan_ssid_bssid *ssid_bssid)
{
	...
	priv->sme_current.mfp = req->use_mfp;
...
#define AUTH_BODY_LEN 6
	...
	PRINTM(MCMND, "Host Mlme: Send Auth\n");
	...
	if (!ret) {
		priv->host_mlme_wait_condition = MFALSE;
		timeout =
			wait_event_timeout(priv->host_mlme_wait_q,
					   priv->host_mlme_wait_condition,
					   1 * HZ);
		if (!timeout) {
			PRINTM(MERROR, "HostMlme: Auth timeout\n");
			priv->host_mlme = MFALSE;
			priv->auth_flag = 0;
			ret = -EFAULT;
		} else {
			PRINTM(MCMND, "HostMlme: Auth done\n");
			priv->auth_flag = 0;
	...
	woal_cfg80211_remain_on_channel_cfg(priv, MOAL_IOCTL_WAIT, MTRUE,
			...
			struct cfg80211_assoc_request *req)
{
	...
	if (!ret && priv->media_connected) {
		...
		PRINTM(MERROR, "wlan: Failed to connect to bssid " MACSTR "\n",
		       MAC2STR(priv->cfg_bssid));
	...
	if (ssid_bssid.assoc_rsp.assoc_resp_len) {
		PRINTM(MCMND, "assoc_resp_len=%d, frame_control=0x%x\n",
 ...
 */
static t_bool
is_wowlan_pattern_supported(
#if CFG80211_VERSION_CODE >= KERNEL_VERSION(3, 12, 0)
				   struct cfg80211_pkt_pattern *pat,
#else
				   struct cfg80211_wowlan_trig_pkt_pattern *pat,
#endif
				   t_s8 *byte_seq)
{
...
woal_cfg80211_suspend(struct wiphy *wiphy, struct cfg80211_wowlan *wow)
{
	...
	for (i = 0; i < wow->n_patterns; i++) {
		...
		if (!is_wowlan_pattern_supported(&wow->patterns[i], byte_seq)) {
		...
		memcpy(mef_entry->filter_item[filt_num].byte_seq, byte_seq,
		       MAX_NUM_BYTE_SEQ);
	...
	if (wow->magic_pkt) {
		mef_cfg.criteria |=
			CRITERIA_UNICAST | CRITERIA_BROADCAST |
			CRITERIA_MULTICAST;
		mef_entry->filter_item[filt_num].fill_flag =
			(FILLING_TYPE | FILLING_REPEAT | FILLING_BYTE_SEQ |
			 FILLING_OFFSET);
		mef_entry->filter_item[filt_num].repeat = 16;
		memcpy(mef_entry->filter_item[filt_num].byte_seq,
	...
	if (!mef_cfg.criteria)
		mef_cfg.criteria = CRITERIA_BROADCAST |
			CRITERIA_UNICAST | CRITERIA_MULTICAST;
