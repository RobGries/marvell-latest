...
struct usb_device_id woal_usb_table[] = {
	...
	{MARVELL_USB_DEVICE
	 (USB8997_VID_1, USB8997_PID_1, "Marvell WLAN USB Adapter")},
	{MARVELL_USB_DEVICE
	 (USB8997_VID_1, USB8997V2_PID_1, "Marvell WLAN USB Adapter")},
	{MARVELL_USB_DEVICE
	 (USB8997_VID_1, USB8997_PID_2, "Marvell WLAN USB Adapter")},
	{MARVELL_USB_DEVICE
	 (USB8997_VID_1, USB8997_PID_3, "Marvell WLAN USB Adapter")},
	{MARVELL_USB_DEVICE
	 (USB8997_VID_1, USB8997_PID_4, "Marvell WLAN USB Adapter")},
	{MARVELL_USB_DEVICE
	 (USB8997_VID_1, USB8997_PID_5, "Marvell WLAN USB Adapter")},
	{MARVELL_USB_DEVICE
	 (USB8997_VID_1, USB8997_PID_6, "Marvell WLAN USB Adapter")},
	/* Terminating entry */
	{},
};
 ...
 */
void
woal_usb_unlink_urb(moal_handle *handle)
{
	struct usb_card_rec *cardp = (struct usb_card_rec *)handle->card;
	int i;
	ENTER();
		...
		if (atomic_read(&cardp->tx_data_urb_pending)) {
			for (i = 0; i < MVUSB_TX_HIGH_WMARK; i++) {
				...
				if (cardp->tx_data2_list[i].urb) {
					usb_kill_urb(cardp->tx_data2_list[i].
						     urb);
				}
			}
		}
...
woal_usb_free(struct usb_card_rec *cardp)
{
	...
	/* Unlink Tx cmd URB */
	if (atomic_read(&cardp->tx_cmd_urb_pending) && cardp->tx_cmd.urb) {
		usb_kill_urb(cardp->tx_cmd.urb);
	}
	/* Unlink Rx cmd URB */
...
woal_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (woal_usb_table[i].idVendor) {
		...
		for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
			...
			if (usb_endpoint_is_bulk_out(endpoint) &&
			    (usb_endpoint_num(endpoint) == MLAN_USB_EP_DATA_CH2)
				) {
				/* We found a bulk out data endpoint */
				PRINTM(MCMND,
